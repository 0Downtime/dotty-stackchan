---
title: Wake Word
description: How Dotty's wake word works today (ESP-SR WakeNet9 "Hi, Stack Chan"), how to switch to a different prebuilt English wake word, and the roadmap to a custom "Hey Dotty" model.
---
...
- **Today:** firmware ships with `SR_WN_WN9_HISTACKCHAN_TTS3` — phrase **"Hi, Stack Chan"** — running on ESP-SR's WakeNet9 + AFE on the ESP32-S3 (PSRAM model). `SR_WN_WN9_HIESP` ("Hi, ESP") remains available as an opt-in alternative.
...
Mic PCM ──▶ AFE (AEC + NS + VAD) ──▶ WakeNet9 ──▶ "wake event" ──▶ Application::HandleWakeWordDetectedEvent
                                          │
                                          └── model: wn9_histackchan (loaded from flash via esp_srmodel_init("model"))
```
...
| Active wake-word model | `firmware/sdkconfig.defaults:18` | `CONFIG_SR_WN_WN9_HISTACKCHAN_TTS3=y` (the line to change). `CONFIG_SR_WN_WN9_HIESP` remains available as an alternative. |
...
| `SR_WN_WN9_HIESP` | "Hi, ESP" *(alternative)* |
| `SR_WN_WN9_HEYKIRA_TTS3` | "Hey, Kira" |

There is no prebuilt "Hey Dotty" — that is what Path B is for. The closest single-syllable-after-"Hey" prebuilts are **Hey Ily**, **Hey Ivy**, and **Hey Kira**. The current shipped default is **"Hi, Stack Chan"**. **"Hi, ESP"** remains the best-trained English model in the catalog, while **"Computer"** is the easiest zero-friction alternative for kids.
...
2. Comment out the active line `CONFIG_SR_WN_WN9_HISTACKCHAN_TTS3=y` (line 18) and uncomment / add the line for the desired key, e.g. `CONFIG_SR_WN_WN9_COMPUTER_TTS=y`. Only one `SR_WN_WN9_*` line should be active at a time.
...
3. Uncheck `Hi, Stack Chan (wn9_histackchan)`. Check the desired entry.
...
Restore `CONFIG_SR_WN_WN9_HISTACKCHAN_TTS3=y` on line 18 of `firmware/sdkconfig.defaults`, delete `firmware/sdkconfig` + `firmware/build/`, rebuild.
