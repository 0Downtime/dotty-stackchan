...
The eight native voice tools in `dotty-pi-ext`: `device_status`, `memory_lookup`, `recall_person`, `remember`, `remember_person`, `think_hard`, `take_photo`, `play_song`. Private external MCP tools use an independent fail-closed allowlist. See [brain.md](./brain.md) for the full tool catalogue.
...
- The `dotty-pi-ext` extension with eight native voice tools and guarded external MCP loading
...
| `dotty-pi-ext/` | Docker host (bind-mounted into dotty-pi) | native voice tools plus guarded external MCP loading |
